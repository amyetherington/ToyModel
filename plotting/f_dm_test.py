...
for i in range(len(lens_name)):
    ...
    kappa_fit_slope = true_profile.best_fit_power_law_slope_with_error_from_radii(radii=radii)[0]

    einstein_radius_best_fit = true_profile.best_fit_power_law_einstein_radius_with_error_from_radii(radii=radii)[0]
    ...
    kappa_best_fit = true_profile.best_fit_power_law_convergence_from_radii(radii=radii)
    ...
    plt.loglog(radii, kappa_lensing, "-.", label="kappa via lensing", color="blue", alpha=0.8)
