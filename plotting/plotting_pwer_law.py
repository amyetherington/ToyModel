...
power_law = profile.SphericalPowerLaw(
    einstein_radius=6.7,
    slope=1.8,
    redshift_lens=0.3,
    redshift_source=0.8,
...
kappa_via_surface_mass = (
    surface_mass_density / power_law_total.critical_surface_density_of_lens
)

no_mask = power_law_total.mask_radial_range_from_radii(
    lower_bound=0, upper_bound=1, radii=radii
)
mask_einstein_radius = power_law_total.mask_radial_range_from_radii(
...
print(
    ...
    power_law_total.three_dimensional_mass_enclosed_within_effective_radius,
)
print(
    "2D mass enclosed in Reff:",
    power_law_total.two_dimensional_mass_enclosed_within_effective_radius,
)
print("Rein:", power_law_total.einstein_radius_in_kpc_from_radii(radii=radii))
print(
    "Rein via regression to kappa:",
    power_law_total.inferred_einstein_radius_with_error_from_mask_and_radii(
        mask=no_mask, radii=radii
    )[0],
)
print(
    "Rein via regression to kappa:",
    power_law_total.inferred_einstein_radius_with_error_from_radii(
        radii=radii
    )[0],
)
...
print(
    ...
    power_law_total.inferred_slope_via_deflections_from_mask_and_radii(
        mask=no_mask, radii=radii
    ),
)
print(
    "slope via regression to kappa:",
    power_law_total.inferred_slope_with_error_from_mask_and_radii(
        mask=no_mask, radii=radii
    )[0],
)
print(
...
print(
    ...
    power_law_total.slope_and_normalisation_via_dynamics(radii=radii)[1],
)
print(
    "slope via lensing and dynamics 2d:",
    power_law_total.slope_and_normalisation_via_2d_mass_and_einstein_mass(radii=radii)[
        1
    ],
)
...
for i in range(len(effective_radii)):
    power_law = profile.SphericalPowerLaw(
        ...
        redshift_lens=0.3,
        redshift_source=0.8,
        effective_radius=effective_radii[i],
    )
    power_law_total = cp.CombinedProfile(profiles=[power_law])
    dynamics = power_law_total.slope_and_normalisation_via_dynamics(radii=radii)[1]
    dynamics_2 = power_law_total.slope_and_normalisation_via_2d_mass_and_einstein_mass(
        radii=radii
    )[1]
