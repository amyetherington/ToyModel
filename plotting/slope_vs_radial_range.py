...
fig_path = plots_path + 'slope_' + str(baryons.slope) + \
           '__ein_' + str(baryons.einstein_radius) + \
           '__scale_r_' + str(DM.scale_radius) + \
           '__kappa_s_' + str(DM.kappa_s)

for i in range(len(upper_bound)):

    kappa_best_fit_slope.append(true_profile.best_fit_power_law_slope_with_error_between_radii_from_radii(
    radii=radii, lower_bound=lower_bound[i], upper_bound=upper_bound[i]
    )[0])
    kappa_best_fit_slope_from_alpha.append(true_profile.best_fit_power_law_slope_via_deflection_angles_between_radii_from_radii(
    radii=radii, lower_bound=lower_bound[i], upper_bound=upper_bound[i]
    ))

    kappa_best_fit_einstein_mass.append(true_profile.best_fit_einstein_mass_in_solar_masses_between_radii_from_radii_and_redshifts(
        radii=radii, lower_bound=lower_bound[i], upper_bound=upper_bound[i], z_l=0.3, z_s=1.0
    )[0])
